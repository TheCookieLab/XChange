...
import info.bitrich.xchangestream.coinbase.CoinbaseChannel;
    ...
    for (JsonNode event : events) {
      ...
      for (JsonNode node : tradeNodes) {
        ...
        Trade trade =
            new Trade.Builder()
                .instrument(pair)
                .id(node.path("trade_id").asText(null))
                .price(asBigDecimal(node, "price"))
                .originalAmount(asBigDecimal(node, "size"))
                .timestamp(asInstant(node.path("time")).map(java.util.Date::from).orElse(null))
                .type(parseOrderSide(node.path("side").asText(null)))
                .build();
        trades.add(trade);
      }
    }
      ...
      for (JsonNode candleNode : candles) {
        ...
        candlesList.add(
            new CandleStick.Builder()
                .open(asBigDecimal(candleNode, "open"))
                .close(asBigDecimal(candleNode, "close"))
                .high(asBigDecimal(candleNode, "high"))
