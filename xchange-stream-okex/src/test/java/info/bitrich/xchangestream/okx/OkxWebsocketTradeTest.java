...
import static info.bitrich.xchangestream.okx.Utils.getMinAmount;
...
@Ignore
public class OkxWebsocketTradeTest {

  private static final Logger LOG = LoggerFactory.getLogger(OkxWebsocketTradeTest.class);
  ...
  public void setUp() {
    ...
    spec.setExchangeSpecificParametersItem(OkxExchange.USE_SANDBOX, true);
    // for REST demo
    spec.setExchangeSpecificParametersItem(OkxExchange.PARAM_SIMULATED, "1");
    exchange =
        ExchangeFactory.INSTANCE.createExchangeWithoutSpecification(OkxStreamingExchange.class);
  ...
  private void tradeTest(Instrument instrument) throws IOException, InterruptedException {
    ...
    CancelOrderParams params =
        new OkxTradeParams.OkxCancelOrderParams(instrument, null, limitOrderUserId);
