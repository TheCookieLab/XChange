...
public class OkxStreamingService extends JsonNettyStreamingService {
  ...
  private OkxSubscriptionTopic getTopic(String channelName) {
    if (channelName.contains(ORDERBOOK5)) {
      return new OkxSubscriptionTopic(ORDERBOOK5, null, null, channelName.replace(ORDERBOOK5, ""));
      ...
      return new OkxSubscriptionTopic(ORDERBOOK, null, null, channelName.replace(ORDERBOOK, ""));
    } else if (channelName.contains(TRADES)) {
      return new OkxSubscriptionTopic(TRADES, null, null, channelName.replace(TRADES, ""));
    } else if (channelName.contains(TICKERS)) {
      return new OkxSubscriptionTopic(TICKERS, null, null, channelName.replace(TICKERS, ""));
