...
import static org.knowm.xchange.okx.dto.trade.OkxOrderFlags.POST_ONLY;
...
@Ignore
public class OkxPrivateDataIntegration {

  private final Logger LOG = LoggerFactory.getLogger(OkxPrivateDataIntegration.class);
  ...
  public void placeLimitOrderGetOpenOrderAndCancelOrder() throws IOException {
    ...
    exchange
        .getTradeService()
        .cancelOrder(new OkxCancelOrderParams(instrument, orderId, userReference));
  }
  ...
  public void feeRates() throws IOException {
    OkxAccountService okxAccountService = ((OkxAccountService) exchange.getAccountService());
    Map<Instrument, Fee> feeMap = okxAccountService.getDynamicTradingFeesByInstrument("SWAP");
    feeMap.forEach(
        (key, value) -> {
  ...
  public void setLeverage() throws IOException {
    ...
    System.out.println(
        "Set leverage 1, for "
            + instrument
            + ", result: "
            + okxAccountService.setLeverage(instrument, 1));
  }
}
